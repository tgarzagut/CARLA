## autonomous driver fails to yield for firetruck 
#################################################
...
def spawn_vehicles_around_ego_vehicles(ego_vehicle, radius, spawn_points, numbers_of_vehicles, world, client):
    ...
    #         # Check if the spawn point is within the defined range
    #         if x_min <= x <= x_max and y_min <= y <= y_max:
    #             # Print the spawn point coordinates if it matches the range
    #             print(f"Spawn point at (x: {x}, y: {y}) within the range.")
    #     return

    if (numbers_of_vehicles == 2):
        print("mhmmmmmm")
        filtered_spawn_points = [
            spawn_point for spawn_point in spawn_points
            if x_min <= spawn_point.location.x <= x_max and y_min <= spawn_point.location.y <= y_max
        ]
        for spawn_point in spawn_points:
            x, y = spawn_point.location.x, spawn_point.location.y
        ...
        np.random.shuffle(filtered_spawn_points)  # shuffle  all the spawn points
        ego_location = ego_vehicle.get_location()
        accessible_points = []
        ...
        vehicle_bps = world.get_blueprint_library().filter('vehicle.carlamotors.firetruck')   # don't specify the type of vehicle

        # keep the spawned vehicle in vehicle_list, because we need to link them with traffic_manager
        print("accs points:" + str(len(accessible_points)))
        print("line 251: " + str(numbers_of_vehicles))
        # for i in range(numbers_of_vehicles):  # generate the free vehicle
        point = accessible_points[0]
        vehicle_bp = np.random.choice(vehicle_bps)
        try:
            vehicle = world.spawn_actor(vehicle_bp, point)
            vehicle_list.append(vehicle)
            actor_list.append(vehicle)
            print("Succesfully spawned car")
        except:
            print('failed')  # if failed, print the hints.
            pass
        # spawn_point = carla.Transform(carla.Location(x=106.01924896240234, y=50.86931228637695, z=0.000000), carla.Rotation(pitch=0.000000, yaw=-180, roll=0.000000))
        # # generate the free vehicle
        # vehicle_bp = world.get_blueprint_library().filter('vehicle.carlamotors.firetruck')
        # if vehicle_bp:
        #     vehicle_bp = vehicle_bp[0]  # select the first blueprint if available
        # else:
        #     print("No firetruck blueprint found!")
        # try:
...
# add those free vehicle into trafficemanager, and set them to autopilot.
    ...
    for v in vehicle_list:  # set every vehicle's mode
        ...
        tm.vehicle_percentage_speed_difference(v, -15) ##speed limit 45
        tm.update_vehicle_lights(v, True)
        if(numbers_of_vehicles == 2):
            carla.VehicleLightState.All = True
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(carla.Location(x=52.4, y=24.9, z=0.6), carla.Rotation(pitch=0, yaw=0, roll=0))
        ...
        # spawn more vehicles
        spawn_points = self.map.get_spawn_points()
        spawn_vehicles_around_ego_vehicles(self.player, radius=50, spawn_points=spawn_points, numbers_of_vehicles=2, world=self.world, client=self.client)
        
        # spawn bikes
        spawn_points = self.map.get_spawn_points()
        ##coordinate
        # spawn_bikes(self.player, radius=50, spawn_points=spawn_points, number_of_bikes=0, world=self.world, client=self.client)
        
        # spawn pedestrians
        spawn_points = self.map.get_spawn_points()
        # spawn_pedestrians(world=self.world, client=self.client, number_of_pedestrians=10)
...
def game_loop(args):
    ...
    try:
        ...
        oldTime = time.time()
        weather = carla.WeatherParameters.ClearNoon # Clear Noon
