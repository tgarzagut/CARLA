...
class World(object):
    ...
    def restart(self, args):
        ...
        if self.player is not None:
            ...
            spawn_point = self.map.get_waypoint(carla.Location(x=50, y=24.404694, z=1.0))
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            #XYZ
            #2 SPAWN PEDESTRIANS AT CROSSWALK
            if loc.x < -95.8 and loc.x > -109 and loc.y < 35 and loc.y > 17 and len(walkers) == 1:
                ...
                spawn_pedestrians(world=world.world, client=client, type_pedestrian="walker.pedestrian.0007", x=-98.9, y=35)
                print("----Exiting the third scenario----\n")
            #####################

            #2 SPAWN PEDESTRIANS AT CROSSWALK
            if loc.x < -76.7 and loc.x > -80 and loc.y < 17.2 and loc.y > 15:
                print("\n----Entering agro vehic scenario...----")
                world.spawn_agro_vehicles()
                print("----Exiting agro vehic scenario----\n")
            #####################
