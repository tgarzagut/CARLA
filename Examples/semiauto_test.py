...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # get current weather when simulation loads up
            ...
            if time.time() - oldTime >= (59) and time.time() - oldTime < (59*2) and weather != static_weather_parameters[12]:
                print("---One minute has passed, spawning random vehicles.---")
                weather = static_weather_parameters[12]
                sim_world.set_weather(weather)
                world.spawn_vehicles_around_ego_vehicles()
                
            # Clear Noon - 2 min marker
            if time.time() - oldTime >= (59*2) and time.time() - oldTime < (59*3) and weather != static_weather_parameters[0]:
            ...
            if time.time() - oldTime >= (59*3) and time.time() - oldTime < (59*4) and weather != static_weather_parameters[5]:
                print("---Three minutes have passed, spawning aggresive vehicles.---")
                weather = static_weather_parameters[5]
                sim_world.set_weather(weather)
                world.spawn_agro_vehicles()
                
            # Cloudy Sunset - 4 min marker
            if time.time() - oldTime >= (59*4) and time.time() - oldTime < (59*5) and weather != static_weather_parameters[8]:
                print("---Four minutes have passed, spawning cautious vehicles.---")
                weather = static_weather_parameters[8]
                sim_world.set_weather(weather)
                world.spawn_cautious_vehicles()
                

            # Soft Rain Noon - 5 min marker
            if time.time() - oldTime >= (59*5) and time.time() - oldTime < (59*6) and weather != static_weather_parameters[6]:
                print("---Five minutes have passed.---")
                print("Attempting to spawn Bikes....")
                weather = static_weather_parameters[6]
                sim_world.set_weather(weather)
                world.spawn_bikes()

            # Wet Sunset - 6 min marker
            if time.time() - oldTime >= (59*6) and time.time() - oldTime < (59*7) and weather != static_weather_parameters[9]:
                print("---Six minutes have passed.---")
                weather = static_weather_parameters[9]
                sim_world.set_weather(weather)

            # Soft Rain Sunset - 7 min marker
            if time.time() - oldTime >= (59*7) and time.time() - oldTime < (59*8) and weather != static_weather_parameters[13]:
                print("---Seven minutes have passed.---")
                weather = static_weather_parameters[13]
                sim_world.set_weather(weather)
            
            # Clear Sunset - 8 min marker
            if time.time() - oldTime >= (59*8) and time.time() - oldTime < (59*9) and weather != static_weather_parameters[7]:
                print("---Eight minutes have passed.---")
                weather = static_weather_parameters[7]
                sim_world.set_weather(weather)

            # Wet Cloudy Sunset - 9 min marker
            if time.time() - oldTime >= (59*9) and time.time() - oldTime < (59*10) and weather != static_weather_parameters[10]:
                print("---Nine minutes have passed.---")
                weather = static_weather_parameters[10]
                sim_world.set_weather(weather)

            # Mid Rainy Noon - 10 min marker
            if time.time() - oldTime >= (59*10) and time.time() - oldTime < (59*11) and weather != static_weather_parameters[4]:
                print("---Ten minutes have passed.---")
                weather = static_weather_parameters[4]
                sim_world.set_weather(weather)

            # Light Wet Noon - 11 min marker
            if time.time() - oldTime >= (59*11) and time.time() - oldTime < (59*12) and weather != static_weather_parameters[2]:
                print("---Eleven minutes have passed.---")
                weather = static_weather_parameters[2]
                sim_world.set_weather(weather)

            # Cloudy Noon - 12 min marker
            if time.time() - oldTime >= (59*12) and time.time() - oldTime < (59*13) and weather != static_weather_parameters[1]:
                print("---Twelve minutes have passed.---")
                weather = static_weather_parameters[1]
                sim_world.set_weather(weather)
            
            # Wet Cloudy Noon - 13 min marker
            if time.time() - oldTime >= (59*13) and time.time() - oldTime < (59*14) and weather != static_weather_parameters[3]:
                print("---Thirteen minutes have passed.---")
                weather = static_weather_parameters[3]
                sim_world.set_weather(weather)

            # Mid Rain Sunset - 14 min marker
            if time.time() - oldTime >= (59*14) and time.time() - oldTime < (59*15) and weather != static_weather_parameters[11]:
                print("Fourteen minutes have passed.")
                weather = static_weather_parameters[11]
                sim_world.set_weather(weather)
            ########################
            ...
            if time.time() - oldTime >= (59*15) and time.time() - oldTime < (59*16):
                print("It has been 15 minutes, scenario ending")
                print(f"A total of {index}/12 destinations were reached.")
                break

            if index == 12 and agent.done():
                print("All 12 destinations have been reached.")
